...
# Page config
st.set_page_config(
    page_title="Submit Contract - California Drywall",
    layout="wide"
)
...
def login_form():
    ...
    st.subheader("Project Manager Login")
    
    with st.form("login_form"):
        email = st.text_input("Email", placeholder="pm.test@caldrywall.com")
        ...
        if submit:
            ...
            with st.spinner("Logging in..."):
                ...
                if auth_response and auth_response.user:
                    ...
                    if profile and profile.get('role') != 'project_manager':
                        st.warning(f"Note: You're logged in as {profile.get('role')}. This form is for Project Managers.")
...
def submission_form():
    ...
    with col1:
        st.title("Submit Contract for Review")
        st.write(f"Welcome, **{st.session_state.user_profile.get('full_name')}**")
    with col2:
        if st.button("Logout"):
            st.session_state.authenticated = False
            st.session_state.user = None
